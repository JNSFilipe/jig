...
For an unexplained failure, intermittent bug, or unsuccessful fix, read an installed `debug/SKILL.md` directly and reuse it once loaded. Otherwise reproduce the failure, trace the cause, and test one explanation before correcting it. Keep diagnostic evidence in the same record; a known, localized cause needs no separate debugging step.
